    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
           ...
           Employee emps= employeelist.Find(delegate (Employee ee)
           {
            ...
            Employee emp = employeelist.Find(empe => empe.ID == 101);//lambda expression
            Console.WriteLine(emp.ID+"  "+emp.Name );


            Employee empn = employeelist.Find((Employee empe) => empe.ID == 101);//lambda expression, optional
            Console.WriteLine(empn.ID + "  " + empn.Name);

            int count = employeelist.Count(ee => ee.Name.StartsWith("A"));
